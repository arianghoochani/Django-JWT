...
from .classes import AuthorizationException,RegisterUserResponse,AuthorizingResponse
...
def tokenVerified(exception,loginRequestHeader):
    ...
    try:
        ...
        if token_type == 'BEARER':
            ...
            logging.info("error={error}  desc={desc}".format(error="type of token is not bearer. type of token is : {token_type}".format(token_type=token_type), desc=exception.exceptionDesc))
    except:
        exception.errorKey = 'invalid_token_error'
        logging.warning("error={error}  desc={desc}".format(error="entered token is invalid", desc=exception.exceptionDesc))
    return token
    ...
    if (token_type.lower() == 'bearer'):
        if (user.is_superuser):
            try:
                ...
                logging.error("error={error}  desc={desc}".format(error=unknownException, desc=exception.exceptionDesc))
        else:
